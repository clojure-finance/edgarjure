...
Output is **long-format** by default (`:shape :long`); pass `:shape :wide` for a pivoted dataset.

Key normalization features:
- **Concept fallback chains** (e.g. `RevenueFromContractWithCustomerExcludingAssessedTax` → `Revenues` → `SalesRevenueNet`) — first concept present in the data wins
- **Duration vs instant filtering** — balance sheet uses instant frames (`:frame` ends in `"I"`); income/cashflow use duration frames
- **Restatement deduplication** — latest `:filed` date wins per `[concept period]` pair (as-reported mode)
- **`:as-of` point-in-time mode** — pass `:as-of "YYYY-MM-DD"` to exclude any filing submitted after that date; essential for backtesting and event studies (see [Point-in-Time Data](#point-in-time-data-and-look-ahead-bias))

Concept vectors are public `def`s — override for non-standard filers:
...
> **Critical for backtesting, event studies, and investment-strategy research.**
> Using the wrong data vintage introduces look-ahead bias — the most common
> source of spurious alpha in quantitative finance research.

### The problem

SEC EDGAR stores *all* revisions of every financial figure.  When Apple restates
FY2021 revenue in their 2023 10-K, the XBRL API returns the restated value for
the FY2021 period.  Without controlling for this, a backtest run today "knows"
2021 figures that weren't available until 2023 — guaranteed look-ahead bias.

### The two modes

| Mode | When to use | How |
|---|---|---|
| **As-reported (default)** | Current screening, fundamental analysis, earnings quality research | `(e/income "AAPL")` — no `:as-of` |
| **Point-in-time** | Backtesting, event studies, panel regressions, any strategy evaluated at a historical date | `(e/income "AAPL" :as-of "2022-01-01")` |

### How it works

Each XBRL observation carries a `:filed` date (when the SEC filing was
submitted).  The `:as-of` filter excludes any observation where
`:filed > as-of-date`, then picks the most-recently-filed survivor per
`[concept period]` pair.  This gives you exactly the data a market participant
had available on that date — no more, no less.

### API

All four financial-statement functions accept `:as-of`:

```clojure
;; Point-in-time income statement: only use filings submitted on or before 2022-01-01
(e/income "AAPL" :as-of "2022-01-01")

;; Point-in-time balance sheet
(e/balance "AAPL" :as-of "2022-01-01")

;; Point-in-time cash flow
(e/cashflow "AAPL" :as-of "2022-01-01")

;; All three at once ? consistent as-of across all statements
(e/financials "AAPL" :as-of "2022-01-01")

;; Combinable with :shape and :form
(e/income "AAPL" :as-of "2022-01-01" :shape :wide :form "10-Q")

;; Via underlying namespace (same options)
(financials/income-statement "AAPL" :as-of "2022-01-01")
```
...
;; Build a point-in-time panel for a portfolio of tickers
;; Each rebalance date gets only the data available at that date

(def tickers ["AAPL" "MSFT" "GOOG" "AMZN"])
(def rebalance-dates ["2019-01-15" "2020-01-15" "2021-01-15" "2022-01-15"])

(for [date rebalance-dates
      ticker tickers]
  {:date   date
   :ticker ticker
   :income (e/income ticker :as-of date :shape :wide)})

;; Each :income dataset contains only information that existed
;; in EDGAR as of that rebalance date ? look-ahead-safe by construction.
```

### What :as-of does NOT protect against

1. **Accounting standard changes (e.g. ASC 606, IFRS 15).**  When a company
   adopts a new revenue standard, it often restates comparative prior periods
   in the *same* filing.  These restated comparatives appear with `:filed` equal
   to the adoption-year filing date, so `:as-of` correctly excludes them for
   earlier dates — but you will then see the old-basis figures for those years,
   which are inconsistent with the new-basis figures post-adoption.
   *Research practice:* restrict your panel to periods after the adoption date,
   or flag the accounting break and model it explicitly as a structural break.

2. **`e/facts` / raw XBRL.**  The raw `get-facts-dataset` / `e/facts` functions
   return all revisions unfiltered.  For look-ahead-safe raw facts, filter the
   returned dataset yourself using the `:filed` column:
   ```clojure
   (-> (e/facts "AAPL" :concept "Assets")
       (ds/filter-column :filed #(not (pos? (compare % "2022-01-01")))))
   ```

3. **The `:filed` date is the SEC submission date**, not the announcement date.
   For very tight event windows (e.g. earnings announcement day), use
   `:as-of` set to a few days after the expected announcement date to avoid
   any edge cases with filing delays.

### Implementation detail

The key private function is `edgar.financials/dedup-point-in-time`.  It is
called by `normalized-statement` whenever `:as-of` is non-nil.  When `:as-of`
is nil it delegates to `dedup-restatements` (always-latest behaviour), so
...
7. **No point-in-time filter in `get-facts-dataset`/`e/facts`.** Raw XBRL facts include all revisions. Deduplication and `:as-of` filtering are applied only in `edgar.financials` statement builders. For look-ahead-safe raw facts, filter the returned dataset manually on the `:filed` column.
...
- **Restatement handling:** ✅ Point-in-time `:as-of` implemented in `edgar.financials`. Remaining: expose `:as-of` on `e/panel` for multi-company point-in-time panels; `:as-of` filter helper for raw `e/facts` datasets.
...
### PROJECT_SUMMARY.md is gitignored

`PROJECT_SUMMARY.md` is listed in `.gitignore` (under "# Internal docs") and must **never** be force-added to git (`git add -f`). It is a local LLM-assistant context file only. If it accidentally gets committed, remove it with `git rm --cached PROJECT_SUMMARY.md`.
